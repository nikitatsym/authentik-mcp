...
__all__ = ["AuthentikClient", "Settings", "client_var", "main", "mcp"]


def main() -> None:
    parser = argparse.ArgumentParser(
        prog="authentik-mcp",
        description="MCP server for Authentik. Serves MCP over stdio unless --http is given.",
    )
    parser.add_argument(
        "--http",
        action="store_true",
    ...
    # A bad credential fails here, not on the first tool call.
    _get_client().check()
